# storybook-gen

<p align="center">
  <img src="public/icon.png" width="120" alt="storybook-gen 图标">
</p>

文本转有声书工具：上传 txt / md，自动拆句逐段合成，合并输出一个连贯的 mp3。支持批量多选上百个文件入队挂机转换。

区别于旧的 `ai-voice-storybook`（依赖本地 CosyVoice 大模型），本工具采用**可插拔 TTS provider** 架构：
- 默认内置小米 **MiMo-V2.5-TTS**（OpenAI 兼容接口，限时免费，无需本地模型）
- 新增引擎只需实现 `TTSProvider` 接口并注册，无需改动管线
- API Key 通过 WebUI 填写，只保存到本地 `.env`，不硬编码进源码

## 功能

- WebUI（`http://localhost:5666`）：配置 API Key、选音色、填语气指令、上传文件或粘贴文本、实时进度
- 批量挂机转换：文件框支持多选上百个 txt/md，入队后按顺序自动转换，产物落盘 `output/`，不触发逐文件下载
- 转换队列：任务级进度统计（待处理/完成/失败/待重试）+ 当前文件段级进度，单个任务失败不中断后续
- **失败自动重试**：只有 API 侧故障（限流 429 / 超时 / 5xx / 响应为空）才重试——单段失败先就地退避重试，整章仍失败则把该文件重新排到队尾、等前面文件转完再试；本地原因（缺 Key、ffmpeg 未装、文件读不到）直接判失败，不白占队列。失败任务保留原文，WebUI 可一键「重试失败任务」补转
- LLM 自动语气（可选）：配置 OpenAI 兼容 LLM 后，自动拆解文本并逐段生成语气指令，驱动 TTS 带语气朗读
- **失败日志带诊断信息**：接口报错时打印状态码、响应体原文、网关响应头（如 `x-mife-upstream-status`）与请求参数，并给出处理建议；同类错误 30s 内只展开一次，不会把控制台刷爆
- 文本预处理：去 markdown 标记（保留标题文字与对话引号），按自然段/句子拆分，每段 ≤300 字
- 合成管线：串行逐段合成（稳、不易限流）→ ffmpeg 合并，段落间自动加 0.25s 静音
- 产物管理：全部自动落盘 `output/`，WebUI 产物卡片可随时查看/下载，客户端断开不丢音频
- 输出规格：24kHz 单声道 64kbps MP3（朗读够用、体积小）

## 快速开始

```bash
npm install
npm start        # 启动后打开 http://localhost:5666
```

在 WebUI 里填入 MiMo API Key（到 [platform.xiaomimimo.com](https://platform.xiaomimimo.com) 控制台获取），
再选择一个或多个 `.txt` / `.md` 文件（或粘贴文本）加入队列，转换在后台按顺序执行，产物保存在 `output/`。

## 配置（.env）

`.env.example` 复制为 `.env` 即可，WebUI 保存 Key 时也会自动写入：

```ini
TTS_PROVIDER=mimo
MIMO_API_KEY=            # 通过 WebUI 填写，勿提交到仓库
MIMO_VOICE=茉莉          # 精品音色：茉莉/冰糖/苏打/白桦/Mia/Chloe/Milo/Dean
PORT=5666

# LLM（OpenAI 兼容）自动语气功能（可选，也可通过 WebUI 填写）
LLM_API_KEY=
LLM_BASE_URL=
LLM_MODEL=
AUTO_STYLE_ENABLED=false

# 失败自动重试（均为可选，留空用默认值）
TTS_SEG_ATTEMPTS=3           # 单段合成总尝试次数（含首次）
TTS_SEG_RETRY_BASE_MS=1500   # 段级退避基数
TTS_SEG_RETRY_MAX_MS=15000   # 段级单次退避上限
TTS_TASK_ATTEMPTS=4          # 单文件总尝试次数（含首次，即最多重排队 3 次）
TTS_TASK_RETRY_BASE_MS=15000 # 任务级退避基数
TTS_TASK_RETRY_MAX_MS=120000 # 任务级单次退避上限
MIMO_TIMEOUT_MS=120000       # 单次 TTS 请求超时，防接口挂死卡住队列

# 失败日志（可选）
TTS_OUTAGE_WARN_AFTER=5      # 连续失败多少次后提示「接口疑似故障」，0 关闭
```

重试参数在服务启动时读取，改完 `.env` 需重启服务。

## 架构

```
src/
  server.ts          # Fastify 入口：WebUI + 批量转换队列 API + 产物列表
  config.ts          # .env 读写（Key 只存这里）
  text.ts            # markdown 清洗 + 拆句
  audio.ts           # ffmpeg 合并 mp3（段落间静音）
  llm.ts             # LLM 自动拆解 + 逐段语气指令（自动语气功能）
  pipeline.ts        # 文本 → 逐段合成（含段级重试）→ 合并 主流程
  errors.ts          # 错误分类 + 诊断信息（API 侧故障 vs 本地/配置问题）
  errorlog.ts        # 失败日志整理：同类错误去重展开 + 持续失败告警
  retry.ts           # 重试工具：指数退避 + 抖动，段级与任务级共用
  providers/
    types.ts         # TTSProvider 接口（新增引擎实现它）
    mimo.ts          # 小米 MiMo-V2.5-TTS
    index.ts         # provider 注册表
public/index.html    # WebUI 单文件
output/              # 生成的 mp3（自动落盘）；.tmp/ 为上传暂存目录，自动清理
```

### 新增 TTS provider

1. 在 `src/providers/` 新建文件，实现 `TTSProvider` 接口（`synthesize()` 返回 mp3 Buffer，`listVoices()` 返回音色列表）。
2. 在 `src/providers/index.ts` 里 `registerProvider(new XxxProvider())`。
3. 在 `.env` 设 `TTS_PROVIDER=xxx`，如需 Key 配置再在 WebUI 或 `config.ts` 补一条保存逻辑。

## API

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/status` | provider 列表、当前选中、Key 是否已配置、LLM 配置状态 |
| POST | `/api/config/apikey` | 保存 provider 的 API Key 到 `.env` |
| POST | `/api/config/llm` | 保存 LLM（Base URL / API Key / Model）配置 |
| POST | `/api/config/autostyle` | 切换自动语气开关 |
| GET | `/api/progress` | 队列状态：当前任务段级进度 + 任务级统计（pending/done/failed）+ 任务明细 |
| POST | `/api/convert` | JSON：`{ text, voice, style, autoStyle }`，入队（粘贴文本） |
| POST | `/api/convert/file` | multipart：`file`（可多文件）+ `voice` + `style` + `autoStyle`，全部入队后立即返回 |
| GET | `/api/output` | 产物列表（`output/` 下 mp3，按修改时间倒序） |
| GET | `/api/output/:name` | 下载指定产物 |

## 批量转换整本小说

WebUI 已内置批量队列：文件框一次多选上百个 txt/md（按选择顺序入队），转换在后台串行执行，
产物自动按原文件名落盘到 `output/`，WebUI「产物」卡片可随时查看和下载，无需逐文件触发下载。

命令行批量同理，循环调 `/api/convert/file` 逐文件入队即可：

```bash
for f in chapters/*.txt; do
  curl -s -F "file=@$f" -F "voice=茉莉" http://localhost:5666/api/convert/file
done
```

注意：队列当前为内存态，服务重启后未完成任务丢失（上传暂存文件一并清理），重新入队即可。

## 失败重试机制

小米接口偶发限流或返回空内容，批量上百个文件时中间挂掉几个是常态。重试分两层，都**只针对 API 侧故障**：

1. **段级重试**（`src/pipeline.ts`）：单段合成失败就地退避重试（默认最多 3 次），
   避免 200 段的一章在第 180 段挂掉就要整章重来。
2. **任务级重试**（`src/server.ts`）：整章仍失败且属 API 侧原因时，把该文件**重新排到队尾**，
   等队列里其它文件转完、退回退避时间到点后再试（默认最多重排队 3 次）。
   WebUI 里这类任务标记为「待重试」，能看到它是第几次重试。

判断依据见 `src/errors.ts`：429 / 408 / 5xx / 请求超时 / 网络中断 / 响应无音频数据 → 可重试；
400（请求本身有问题）、401（Key 无效）、以及本地原因（缺 Key、ffmpeg 失败、文件读不到）→ 直接判失败，
不浪费队列时间。未识别的异常一律按本地问题处理。

重试次数用尽后任务停在「失败」，但原文仍保留在内存里，WebUI 会显示「重试失败任务」按钮，
可一键把它们重新排到队尾（例如换好新 Key 或接口恢复后补转）。

## 失败日志怎么看

接口报错时，控制台会把「为什么失败」摊开，而不是只给一句 `HTTP 500`：

```
[失败] 第19章.md（API 侧失败，重排队 3 次仍未成功；卡在 1/273 段，前 0 段已合成）
  MiMo TTS 失败：HTTP 500 Internal Server Error
  ├ 状态码：500 Internal Server Error（可重试）
  ├ 耗时：214ms
  ├ 错误码：500
  ├ 响应体：{"error":{"code":"500","message":"Internal Server Error","param":"","type":"InternalServerError"}}
  ├ 响应头：x-mife-upstream-status=500 · server=MiFE/3.4.34 · content-type=application/json
  ├ 请求参数：model=mimo-v2.5-tts · voice=茉莉 · format=mp3 · textLen=288 · styleLen=41 · styleSource=默认
  └ 提示：服务端内部错误，与本地配置无关；若持续出现说明接口正在故障，重试通常无效，可稍后再试或联系官方支持
```

几处设计上的取舍：

- **响应体原文一律保留**（超过 2000 字符才截断）。之前的实现只取 `error.message`，而小米接口 500 时
  这句话恰好就是 `Internal Server Error`，等于什么都没说。非 JSON 的网关报错页也会原样打出来。
- **带上网关响应头**。`x-mife-upstream-status: 500` 这类信息能区分「我们请求发错了」和「上游服务挂了」。
- **请求参数只记长度、不记正文**（小说内容不进日志）。只有 4xx 这种「请求本身有问题」的场景，
  才会附一小段文本片段，方便定位是哪一段触发的。
- **同类错误去重**：同一类错误（同状态码 + 同错误体）30s 内只完整展开一次，其余打一行
  `（同类错误第 N 次，细节见上方首次输出）`。一本 200 段的书连续 500 时，日志从上千行压到几十行，
  最后判失败那一次仍会强制完整展开，保证现场不丢。
- **持续失败告警**：连续失败 `TTS_OUTAGE_WARN_AFTER` 次（默认 5）且期间没有任何一段成功，
  会提示「接口疑似故障，重试大概率无效」，并说明失败任务可用「重试失败任务」补转，不必重新上传。

## 已知限制 / TODO

- 队列为内存态，不支持断点续转；如需可在任务入队时持久化到磁盘
- 重试上限内仍失败的任务会停在「失败」状态，需在接口恢复后手动补转或重新上传
- 接口整体宕机（如全线 500）时没有熔断：每个文件仍会走完「段级 3 次 + 任务级 3 次」重试后才判失败，
  只是比平时慢，不会丢文件。需要的话可加「连续失败 N 次即暂停队列」的熔断开关
- MiMo 语速参数字段未确认，`speed` 暂未下发（见 `providers/mimo.ts` 内 TODO）